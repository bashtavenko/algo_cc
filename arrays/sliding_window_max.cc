...
// O(n)
std::vector<int32_t> SlidingWindowMax(const std::vector<int32_t>& nums, size_t k) {
  std::vector<int32_t> result;
  ...
  // You throw out anyone who's too old (outside a window).
  ...
  // If a soldier gets too old (outside the k - range),
  //  they exit front quietly
  std::deque<int32_t> dq;
  for (size_t i = 0; i < nums.size(); ++i) {
    // Pop front elements outside the current window
    while (!dq.empty() && static_cast<size_t>(dq.front()) <= i - k) dq.
        pop_front();
