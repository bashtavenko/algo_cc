...
int32_t Run(int32_t y, int32_t x, std::vector<std::vector<int32_t>>* p) {
  if (y == 0 && x == 0) return 1;

  std::vector<std::vector<int32_t>>& ways = *p;

  if (ways[y][x] == 0) {
    // We just initialized the array with 0. Now fill it.
    int32_t ways_top = y == 0 ? 0 : Run(y - 1, x, p);
    int32_t ways_left = x == 0 ? 0 : Run(y, x - 1, p);
