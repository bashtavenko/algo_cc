...
std::vector<std::vector<int32_t>> FindQueensPosition(int32_t n) {
  ...
  auto is_valid = [&](const std::vector<int32_t>& col_placement) {
    const int32_t row_id = col_placement.size() - 1;
    for (int32_t row = 0; row < row_id; ++row) {
  ...
  std::function<void(int32_t, std::vector<int32_t>*)> solve =
      [&](int32_t row, std::vector<int32_t>* col_placement) {
        if (row == n) {
